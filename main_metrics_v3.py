...
early_stopping = tf.keras.callbacks.EarlyStopping(
    ...
    min_delta=0.0000001,  # Minimum change in the monitored quantity to qualify as an improvement
    patience=5,  # Number of epochs with no improvement after which training will be stopped
    verbose=1,  # Verbosity mode. Set to 1 to see messages about the early stopping process
    mode='min'  # Mode for the monitored quantity ('min' for loss, 'max' for accuracy, etc.)
)

NN_model = Sequential()
NN_model.add(Dense(64, input_dim=2, activation='relu'))  # Adjust input_dim as per the number of input features
NN_model.add(Dense(32, activation='relu'))
NN_model.add(Dense(1))  # Output layer with a single node for regression

# Compile the model
NN_model.compile(optimizer='adam', loss='mean_squared_error')  # Using mean squared error loss for regression

# Fit the model to the data
NN_model.fit(X_NN, y_NN, epochs=400, batch_size=4, callbacks=[early_stopping])  # Adjust epochs and batch_size as needed

y_NN_pred = NN_model.predict(X_NN)

plt.scatter(y,y_NN_pred)
