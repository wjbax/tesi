# -*- coding: utf-8 -*-
"""
Created on Mon May 27 09:34:59 2024
...
subset_GT_0 = []
name_GT_0 = []
for p in tqdm(uncertainty_metric):
    UNC_METRIC = uncertainty_metric[p]['name']
    for dataset in tqdm(dataset_dict):
        for subset in tqdm(['val', 'test']):
            ...
            subset_save_path = "D:/DATASET_Tesi_Marzo2024_RESULTS_V6/"+dataset+"/"+subset+"/"
            ...
            for name in tqdm(image_list):
                ...
                save_path = "D:/DATASET_Tesi_Marzo2024_RESULTS_V6/"+dataset+"/"+subset+"/"+name+"/"
                ...
                if np.sum(GT_mask_orig) == 0:
                    dataset_GT_0.append(dataset)
                    subset_GT_0.append(subset)
                    name_GT_0.append(name)
                    continue
                
                list_of_images_to_use.append(name)
    ...
    # %% SOFTMAX MATRIX GENERATOR
                ...
                #creo shannon entropy map CON NOMI SBAGLIATI MA YOLO
                
                bin_ent_map_c1_PERT = uncertainty_metric[p]['function'](softmax_matrix_PERT[:,:,0,:])
                bin_ent_map_c2_PERT = uncertainty_metric[p]['function'](softmax_matrix_PERT[:,:,1,:])
                bin_ent_map_c1_MC = uncertainty_metric[p]['function'](softmax_matrix_MC[:,:,0,:])
                bin_ent_map_c2_MC = uncertainty_metric[p]['function'](softmax_matrix_MC[:,:,1,:])
    
                max_value = np.max([
                    np.max(bin_ent_map_c1_PERT),
                    np.max(bin_ent_map_c2_PERT),
                    np.max(bin_ent_map_c1_MC),
                    np.max(bin_ent_map_c2_MC)])            
                ...
                if np.isnan(max_value): max_value = 1
                Th_array = np.arange(0,max_value,max_value/100)
                for Th in Th_array:
                ...
                plt.plot(Th_array,dice_c1_MC, label = 'c1 MC')
                plt.plot(Th_array,dice_c2_MC, label = 'c2 MC')
                plt.plot(Th_array,dice_c1_PERT, label = 'c1 PERT')
                plt.plot(Th_array,dice_c2_PERT, label = 'c2 PERT')
                plt.axhline(y = dice(SI_mask_C1,GT_mask_C1), color = 'r', linestyle = '-', label = 'c1')
                plt.axhline(y = dice(SI_mask_C2,GT_mask_C2), color = 'g', linestyle = '--', label = 'c2')
                plt.xlim(0,max_value)
                plt.ylim(top=1)
                plt.legend(['c1 MC','c2 MC', 'c1 PERT', 'c2 PERT'])
                plt.savefig(save_path + UNC_METRIC + "dice.png")
                plt.close()
                
                plt.figure(figsize=(10,10))
                plt.plot(Th_array,precision_c1_MC, label = 'c1 MC')
                plt.plot(Th_array,precision_c2_MC, label = 'c2 MC')
                plt.plot(Th_array,precision_c1_PERT, label = 'c1 PERT')
                plt.plot(Th_array,precision_c2_PERT, label = 'c2 PERT')
                plt.axhline(y = precision(SI_mask_C1,GT_mask_C1), color = 'r', linestyle = '-', label = 'c1')
                plt.axhline(y = precision(SI_mask_C2,GT_mask_C2), color = 'g', linestyle = '--', label = 'c2')
                plt.xlim(0,max_value)
                plt.ylim(top=1)
                plt.legend()
                plt.savefig(save_path + UNC_METRIC + "precision.png")
                plt.close()
                
                plt.figure(figsize=(10,10))
                plt.plot(Th_array,recall_c1_MC, label = 'c1 MC')
                plt.plot(Th_array,recall_c2_MC, label = 'c2 MC')
                plt.plot(Th_array,recall_c1_PERT, label = 'c1 PERT')
                plt.plot(Th_array,recall_c2_PERT, label = 'c2 PERT')
                plt.axhline(y = recall(SI_mask_C1,GT_mask_C1), color = 'r', linestyle = '-', label = 'c1')
                plt.axhline(y = recall(SI_mask_C2,GT_mask_C2), color = 'g', linestyle = '--', label = 'c2')
                plt.xlim(0,max_value)
                plt.ylim(top=1)
                plt.legend(['c1 MC','c2 MC', 'c1 PERT', 'c2 PERT'])
                plt.savefig(save_path + UNC_METRIC + "recall.png")
                plt.close()
            ...
            plt.subplot(131)
            plt.plot(list_of_images_to_use,th_max_dice_per_image_MC_C1)
            plt.title("Dice")
            plt.subplot(132)
            plt.plot(list_of_images_to_use,th_max_precision_per_image_MC_C1)
            plt.title("Precision")
            plt.subplot(133)
            plt.plot(list_of_images_to_use,th_max_recall_per_image_MC_C1)
            plt.title("Recall")
            plt.savefig(subset_save_path+ UNC_METRIC+"threshold_per_image_MC_C1.png")
            plt.close()
            ...
            plt.subplot(131)
            plt.plot(list_of_images_to_use,th_max_dice_per_image_MC_C2)
            plt.title("Dice")
            plt.subplot(132)
            plt.plot(list_of_images_to_use,th_max_precision_per_image_MC_C2)
            plt.title("Precision")
            plt.subplot(133)
            plt.plot(list_of_images_to_use,th_max_recall_per_image_MC_C2)
            plt.title("Recall")
            plt.savefig(subset_save_path+ UNC_METRIC +"threshold_per_image_MC_C2.png")
            plt.close()
            ...
            plt.subplot(131)
            plt.plot(list_of_images_to_use,th_max_dice_per_image_PERT_C1)
            plt.title("Dice")
            plt.subplot(132)
            plt.plot(list_of_images_to_use,th_max_precision_per_image_PERT_C1)
            plt.title("Precision")
            plt.subplot(133)
            plt.plot(list_of_images_to_use,th_max_recall_per_image_PERT_C1)
            plt.title("Recall")
            plt.savefig(subset_save_path + UNC_METRIC +"threshold_per_image_PERT_C1.png")
            plt.close()
            ...
            plt.subplot(131)
            plt.plot(list_of_images_to_use,th_max_dice_per_image_PERT_C2)
            plt.title("Dice")
            plt.subplot(132)
            plt.plot(th_max_precision_per_image_PERT_C2)
            plt.title("Precision")
            plt.subplot(133)
            plt.plot(list_of_images_to_use,th_max_recall_per_image_PERT_C2)
            plt.title("Recall")
            plt.savefig(subset_save_path+ UNC_METRIC +"threshold_per_image_PERT_C2.png")
            plt.close()
            
            dict_thresholds = {
                'Image': list_of_images_to_use,
            ...
            df_th.to_csv(subset_save_path+ UNC_METRIC +"Max_Thresholds_per_image.csv",index=False)
            
list_of_0_GT_images = {
    'dataset': dataset_GT_0,
    'subset': subset_GT_0,
    'name': name_GT_0
    }      

df_list_0_GT = pd.DataFrame(list_of_0_GT_images)
df_list_0_GT.to_csv("D:/DATASET_Tesi_Marzo2024_RESULTS_V6/Lista_immagini_con_GT_0.csv",index=False)