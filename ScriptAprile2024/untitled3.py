# -*- coding: utf-8 -*-
"""
Created on Mon May 27 14:49:38 2024
...
path = "D:/DATASET_Tesi_marzo2024/Liver HE steatosis/k-net+swin/TEST_3classes/RESULTS_perturbation/test/mask/1004289_35.png/"
mask_union_PERT_C2 = np.zeros((DIM[0],DIM[1]),dtype=bool)
for n in os.listdir(path):
    current_mask = np.array(Img.open(path+n))
    current_mask = current_mask > 128
    mask_union_PERT_C2 = mask_union_PERT_C2 | current_mask
...
MASK_TO_USE_TO_REDUCE = mask_union_PERT_C2
...
for i in range(n_objects_PERT):
    ...
    unc_map_matrix_PERT = np.multiply(current_mask,BRUM)
...
SI_dice = dice(SI_mask_C2,GT_mask_C2)
...
for th in th_range:
    ...
    mask_unione_reduced = np.zeros_like(mask_union_PERT_C2)
    mask_unione_reduced[mask_th>0] = mask_union_PERT_C2[mask_th>0]
    
    dice_th_mask_unione = dice(mask_unione_reduced,GT_mask_orig_2c)
    dice_array_PERT_mask_unione.append(dice_th_mask_unione)

# mask_temp = (np.multiply(mask_union_PERT_C2,mask_avg_C2_PERT)).astype(bool)
