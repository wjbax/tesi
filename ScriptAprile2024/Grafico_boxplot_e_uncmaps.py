...
#%% SOFTMAX MATRIX GENERATOR
            ...
            boxplot_generator(dice_GTSI_C1,dice_MC_C1,dice_PERT_C1,TOT_dice_C1,fig_save_path+"boxplot_C1.png",name,dataset,subset,1)
            boxplot_generator(dice_GTSI_C2,dice_MC_C2,dice_PERT_C2,TOT_dice_C2,fig_save_path+"boxplot_C2.png",name,dataset,subset,2)
            # uncertainty maps generation
            bin_ent_map_c1_PERT = binary_entropy_map(softmax_matrix_PERT[:,:,1,:])
            sha_ent_map_c1_PERT = shannon_entropy_map(softmax_matrix_PERT[:,:,1,:])
            std_map_c1_PERT = std_map(softmax_matrix_PERT[:,:,1,:])
            dice_mat_map_c1_PERT = dice_mat_map(seg_matrix_PERT_c1, N)
            
            bin_ent_map_c1_MC = binary_entropy_map(softmax_matrix_MC[:,:,1,:])
            sha_ent_map_c1_MC = shannon_entropy_map(softmax_matrix_MC[:,:,1,:])
            std_map_c1_MC = std_map(softmax_matrix_MC[:,:,1,:])
            dice_mat_map_c1_MC = dice_mat_map(seg_matrix_MC_c1, N)
        
            bin_ent_map_c2_PERT = binary_entropy_map(softmax_matrix_PERT[:,:,2,:])
            sha_ent_map_c2_PERT = shannon_entropy_map(softmax_matrix_PERT[:,:,2,:])
            std_map_c2_PERT = std_map(softmax_matrix_PERT[:,:,2,:])
            dice_mat_map_c2_PERT = dice_mat_map(seg_matrix_PERT_c2, N)
        
            bin_ent_map_c2_MC = binary_entropy_map(softmax_matrix_MC[:,:,2,:])
            sha_ent_map_c2_MC = shannon_entropy_map(softmax_matrix_MC[:,:,2,:])
            std_map_c2_MC = std_map(softmax_matrix_MC[:,:,2,:])
            dice_mat_map_c2_MC = dice_mat_map(seg_matrix_MC_c2, N)
#%%
            unc_maps_plots(dataset, subset, name, original_image, GT_mask_orig, bin_ent_map_c1_MC, sha_ent_map_c1_MC, std_map_c1_MC, dice_mat_map_c1_MC, bin_ent_map_c2_MC, sha_ent_map_c2_MC, std_map_c2_MC, dice_mat_map_c2_MC, fig_save_path+"unc_maps_plots_MC.png")
            unc_maps_plots(dataset, subset, name, original_image, GT_mask_orig, bin_ent_map_c1_PERT, sha_ent_map_c1_PERT, std_map_c1_PERT, dice_mat_map_c1_PERT, bin_ent_map_c2_PERT, sha_ent_map_c2_PERT, std_map_c2_PERT, dice_mat_map_c2_PERT, fig_save_path+"unc_maps_plots_PERT.png")
        ...
        plt.axhline(y=dice_GTSI_C1, linewidth=1, color='r')
        plt.title("Mean Dice per dataset: " + dataset + ", subset: " + subset + ", class 1")
        plt.legend(labels=['Monte Carlo', 'Perturbations','Dice value Single Inference'])
        plt.xlabel('Images')
        ...
        plt.axhline(y=dice_GTSI_C2, linewidth=1, color='r')
        plt.title("Mean Dice per dataset: " + dataset + ", subset: " + subset + ", class 2")
        plt.legend(labels=['Monte Carlo', 'Perturbations','Dice value Single Inference'])
        plt.xlabel('Images')
