# -*- coding: utf-8 -*-
"""
Created on Sun Jun 30 15:27:07 2024
...
for diff_type in tqdm(["PERT","MC"]):
    ...
    for subset in tqdm(["test", "val"]):
        ...
        for image in tqdm(os.listdir(general_dataset_path + "DATASET_3classes/" + subset + "/" + "manual/")):
            ...
            for th in th_range:
                C1_mask_th_union = mask_union_C1 & (unc_map_C1<=th)
                C1_mask_th_avg = mask_avg_C1 & (unc_map_C1<=th)
                C1_mask_th_SI = SI_mask_C1 & (unc_map_C1<=th)
                
                C2_mask_th_union = mask_union_C2 & (unc_map_C2<=th)
                C2_mask_th_avg = mask_avg_C2 & (unc_map_C2<=th)
                C2_mask_th_SI = SI_mask_C2 & (unc_map_C2<=th)
            ...
            plt.legend()
            plt.savefig(path_to_save_figure + image[:-4] + "_dicerecallprecision_x_threshold_C1.png")
            plt.close()
            ...
            plt.legend()
            plt.savefig(path_to_save_figure + image[:-4] + "_dicerecallprecision_x_threshold_C2.png")
            plt.close()
        
        C1_df_gen = pd.DataFrame.from_dict(C1_dict_max_values)
        C1_df_gen.to_csv(path_to_save_figure + "DATAFRAME_max_values_C1.csv", index=True)
        ...
        plt.savefig(path_to_save_figure + "union_mask_optimal_threhsolds_C1.png")
        plt.close()
        ...
        plt.savefig(path_to_save_figure + "avg_mask_optimal_threhsolds_C1.png")
        plt.close()
        ...
        plt.savefig(path_to_save_figure + "SI_mask_optimal_threhsolds_C1.png")
        plt.close()
        
    
        C2_df_gen = pd.DataFrame.from_dict(C2_dict_max_values)
        C2_df_gen.to_csv(path_to_save_figure + "DATAFRAME_max_values_C2.csv", index=True)
        ...
        plt.savefig(path_to_save_figure + "union_mask_optimal_threhsolds_C2.png")
        plt.close()
        ...
        plt.savefig(path_to_save_figure + "avg_mask_optimal_threhsolds_C2.png")
        plt.close()
        ...
        plt.savefig(path_to_save_figure + "SI_mask_optimal_threhsolds_C2.png")
        plt.close()