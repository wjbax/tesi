# -*- coding: utf-8 -*-
"""
Created on Mon Mar 18 19:47:50 2024
...
#%%
def std_map(softmax_matrix):
    shape = np.shape(softmax_matrix)
    std_map = np.zeros([shape[0],shape[1]])
    for i in range(shape[0]):
        for j in range(shape[1]):
            ij_vector = softmax_matrix[i,j,1,:]
            std_ij = np.nanstd(ij_vector)
            std_map[i,j] = std_ij
    return std_map
...
for GT_seg_name in tqdm(os.listdir(GT_seg_dir)):
    ...
    # Creo std_map
    std_map_temp = std_map(softmax_matrix)
    GT_std = std_map_temp * GT_mask
    x_metrica_std_map.append(100*(GT_std>0).sum()/(416*416))
    x_metrica_std_map_sum.append(100*(GT_std).sum()/(416*416))
    ...
    path_to_save_figures = "C:/Users/willy/Desktop/Tesi_v2/tesi/data_saves/datset_corrente/Figures/"
    ...
    plt.subplot(242).title.set_text("std Map")
    plt.imshow(std_map_temp)
    plt.colorbar()
    plt.subplot(243).title.set_text("GT mask")
    plt.imshow(GT_mask)
    plt.subplot(244).title.set_text("std Map x GT mask")
    plt.imshow(GT_std)
    plt.colorbar()
    ...
    plt.show()
    plt.savefig(path_to_save_figures+GT_seg_name[:-4]+"_figures.png")
    plt.close()
...
metrics_df_path = "C:/Users/willy/Desktop/Tesi_v2/tesi/data_saves/datset_corrente/"

index_array = np.arange(1,51)
metrics_dict = {'index':index_array,
                'image_tracker':image_tracker, 
                'x_metrica_std_map_sum':x_metrica_std_map_sum,
...
metrics_df.to_csv(metrics_df_path+"metrics_v7.csv",index=False)

#%%

# plt.figure(figsize=(40,10))
# plt.subplot(141).title.set_text("Metrica basata su std")
# plt.plot(index_array,x_metrica_std_map,'b')
...
# plt.plot(index_array,maxminscal(x_metrica_std_map),'b')
# plt.plot(index_array,maxminscal(x_metrica_ent_map),'tab:orange')
# plt.plot(index_array,maxminscal(x_metrica_dice_multi),'g')
# plt.legend(['metrica std_map','metrica ent_map','metrica_matrice_dice'],loc='upper right')
...
# plt.figure(figsize=(40,10))
# plt.subplot(141).title.set_text("Metrica basata su std")
# plt.scatter(mean_dice_for_image,x_metrica_std_map,c='b')
# plt.xlabel("Mean Dice (su 20)")
# plt.ylabel("Metrica basata su std")
# plt.subplot(142).title.set_text("Metrica basata su Entropy")
...
# plt.subplot(144).title.set_text("Overview metriche (normalizzate)")
# plt.scatter(maxminscal(mean_dice_for_image),maxminscal(x_metrica_std_map),c='b')
...
# plt.xlabel("Mean Dice (su 20)")
# plt.legend(['metrica std_map','metrica ent_map','metrica_matrice_dice'],loc='upper left')
...
# plt.subplot(131).title.set_text("Confronto std / Entropy")
# plt.scatter(x_metrica_std_map,x_metrica_ent_map)
# plt.xlabel("Metrica basata su std")
# plt.ylabel("Metrica basata su Entropy")
# plt.subplot(132).title.set_text("Confronto std / Dice Matrix")
# plt.scatter(x_metrica_std_map,x_metrica_dice_multi)
# plt.xlabel("Metrica basata su std")
# plt.ylabel("Metrica basata su matrice dei dice")
...
plt.figure(figsize=(40,10))
plt.subplot(141).title.set_text("Metrica basata su std")
plt.plot(index_array,x_metrica_std_map_sum,'b')
plt.subplot(142).title.set_text("Metrica basata su Entropy")
plt.plot(index_array,x_metrica_ent_map_sum,'tab:orange' )
plt.subplot(143).title.set_text("Metrica basata su matrice dei dice")
plt.plot(index_array,x_metrica_dice_multi,'g')
plt.subplot(144).title.set_text("Overview metriche (normalizzate)")
plt.plot(index_array,maxminscal(x_metrica_std_map_sum),'b')
...
plt.figure(figsize=(40,10))
plt.subplot(141).title.set_text("Metrica basata su std")
plt.scatter(mean_dice_for_image,x_metrica_std_map_sum,c='b')
plt.xlabel("Mean Dice (su 20)")
plt.ylabel("Metrica basata su std")
plt.subplot(142).title.set_text("Metrica basata su Entropy")
...
plt.ylabel("Metrica basata su matrice dei dice")
plt.subplot(144).title.set_text("Overview metriche (normalizzate)")
plt.scatter(maxminscal(mean_dice_for_image),maxminscal(x_metrica_std_map_sum),c='b')
...
plt.figure(figsize=(30,10))
plt.subplot(131).title.set_text("Confronto std / Entropy")
plt.scatter(x_metrica_std_map_sum,x_metrica_ent_map_sum)
plt.xlabel("Metrica basata su std")
plt.ylabel("Metrica basata su Entropy")
plt.subplot(132).title.set_text("Confronto std / Dice Matrix")
plt.scatter(x_metrica_std_map_sum,x_metrica_dice_multi)
plt.xlabel("Metrica basata su std")
plt.ylabel("Metrica basata su matrice dei dice")
plt.subplot(133).title.set_text("Confronto Dice Matrix / Entropy")
...
#%%
mega_matrici_path = "C:/Users/willy/Desktop/Tesi_v2/tesi/data_saves/datset_corrente/"
