# -*- coding: utf-8 -*-
"""
Created on Sat Jul 20 18:20:49 2024
...
for dataset in ["Renal PAS glomeruli/", "Renal PAS tubuli/"]:
    ...
    for subset in tqdm(["test", "val"]):
        ...
        for image in tqdm(os.listdir(general_dataset_path + "DATASET/" + subset + "/" + "manual/")):
            ...
            MC_mask_union_C1,MC_mask_union_C2 = wjb_functions.mask_splitter(mask_union_MC)
            mask_union_PERT = wjb_functions.mask_union_gen_3c(diff_path_3c_PERT)/255
            PERT_mask_union_C1,PERT_mask_union_C2 = wjb_functions.mask_splitter(mask_union_PERT)
            ...
            PERT_label_image_C1 = measure.label(PERT_mask_union_C1)
            PERT_n_objects_C1 = PERT_label_image_C1.max()
            PERT_mask_matrix_C1 = np.zeros([PERT_label_image_C1.shape[0],PERT_label_image_C1.shape[1],PERT_n_objects_C1])
            PERT_tau_array_mean_C1 = []
            PERT_tau_array_mode_C1 = []
            for i in range(PERT_n_objects_C1):
                current_mask = np.copy(PERT_label_image_C1)
                current_mask[current_mask!=i+1] = 0
                max_mask = np.max(current_mask)
                if max_mask == 0: max_mask = 1
                current_mask = current_mask/max_mask
                PERT_mask_matrix_C1[:,:,i] = current_mask
                unc_map_temp_2c = np.multiply(current_mask,unc_map_C1_PERT)
                ...
                PERT_tau_array_mean_C1.append(tau_i_2c_mean)
                PERT_tau_array_mode_C1.append(tau_i_2c_mode)
                
            PERT_tau_array_mean_C1 = np.array(PERT_tau_array_mean_C1)
            PERT_tau_array_mode_C1 = np.array(PERT_tau_array_mode_C1)  
            
            PERT_label_image_C2 = measure.label(PERT_mask_union_C2)
            PERT_n_objects_C2 = PERT_label_image_C2.max()
            PERT_mask_matrix_C2 = np.zeros([PERT_label_image_C2.shape[0],PERT_label_image_C2.shape[1],PERT_n_objects_C2])
            PERT_tau_array_mean_C2 = []
            PERT_tau_array_mode_C2 = []
            for i in range(PERT_n_objects_C2):
                current_mask = np.copy(PERT_label_image_C2)
                current_mask[current_mask!=i+1] = 0
                max_mask = np.max(current_mask)
                if max_mask == 0: max_mask = 1
                current_mask = current_mask/max_mask
                PERT_mask_matrix_C2[:,:,i] = current_mask
                unc_map_temp_2c = np.multiply(current_mask,unc_map_C2_PERT)
                tau_i_2c_mean = np.nanmean(unc_map_temp_2c[unc_map_temp_2c>0])
                tau_i_2c_mode = wjb_functions.wjmode(unc_map_temp_2c[unc_map_temp_2c>0])
                PERT_tau_array_mean_C2.append(tau_i_2c_mean)
                PERT_tau_array_mode_C2.append(tau_i_2c_mode)
                
            PERT_tau_array_mean_C2 = np.array(PERT_tau_array_mean_C2)
            PERT_tau_array_mode_C2 = np.array(PERT_tau_array_mode_C2)  
            
            
            MC_label_image_C1 = measure.label(MC_mask_union_C1)
            MC_n_objects_C1 = MC_label_image_C1.max()
            MC_mask_matrix_C1 = np.zeros([MC_label_image_C1.shape[0],MC_label_image_C1.shape[1],MC_n_objects_C1])
            MC_tau_array_mean_C1 = []
            MC_tau_array_mode_C1 = []
            for i in range(MC_n_objects_C1):
                current_mask = np.copy(MC_label_image_C1)
                current_mask[current_mask!=i+1] = 0
                max_mask = np.max(current_mask)
                if max_mask == 0: max_mask = 1
                current_mask = current_mask/max_mask
                MC_mask_matrix_C1[:,:,i] = current_mask
                unc_map_temp_2c = np.multiply(current_mask,unc_map_C1_MC)
                tau_i_2c_mean = np.nanmean(unc_map_temp_2c[unc_map_temp_2c>0])
                tau_i_2c_mode = wjb_functions.wjmode(unc_map_temp_2c[unc_map_temp_2c>0])
                MC_tau_array_mean_C1.append(tau_i_2c_mean)
                MC_tau_array_mode_C1.append(tau_i_2c_mode)
                
            MC_tau_array_mean_C1 = np.array(MC_tau_array_mean_C1)
            MC_tau_array_mode_C1 = np.array(MC_tau_array_mode_C1)  
            
            MC_label_image_C2 = measure.label(MC_mask_union_C2)
            MC_n_objects_C2 = MC_label_image_C2.max()
            MC_mask_matrix_C2 = np.zeros([MC_label_image_C2.shape[0],MC_label_image_C2.shape[1],MC_n_objects_C2])
            MC_tau_array_mean_C2 = []
            MC_tau_array_mode_C2 = []
            for i in range(MC_n_objects_C2):
                current_mask = np.copy(MC_label_image_C2)
                current_mask[current_mask!=i+1] = 0
                max_mask = np.max(current_mask)
                if max_mask == 0: max_mask = 1
                current_mask = current_mask/max_mask
                MC_mask_matrix_C2[:,:,i] = current_mask
                unc_map_temp_2c = np.multiply(current_mask,unc_map_C2_MC)
                tau_i_2c_mean = np.nanmean(unc_map_temp_2c[unc_map_temp_2c>0])
                tau_i_2c_mode = wjb_functions.wjmode(unc_map_temp_2c[unc_map_temp_2c>0])
                MC_tau_array_mean_C2.append(tau_i_2c_mean)
                MC_tau_array_mode_C2.append(tau_i_2c_mode)
                
            MC_tau_array_mean_C2 = np.array(MC_tau_array_mean_C2)
            MC_tau_array_mode_C2 = np.array(MC_tau_array_mode_C2)
        ...
        #     for th in th_range:
        #         mask_th_union_C1_MC = MC_mask_union_C1.astype(bool) & (unc_map_C1_MC<=th)
        #         union_th_dice_array_C1_MC.append(wjb_functions.dice(mask_th_union_C1_MC, GT_mask_C1))
        #         mask_th_union_C2_MC = MC_mask_union_C2.astype(bool) & (unc_map_C2_MC<=th)
        #         union_th_dice_array_C2_MC.append(wjb_functions.dice(mask_th_union_C2_MC, GT_mask_C2))
        #         mask_th_union_C1_PERT = PERT_mask_union_C1.astype(bool) & (unc_map_C1_PERT<=th)
        #         union_th_dice_array_C1_PERT.append(wjb_functions.dice(mask_th_union_C1_PERT, GT_mask_C1))
        #         mask_th_union_C2_PERT = PERT_mask_union_C2.astype(bool) & (unc_map_C2_PERT<=th)
