# -*- coding: utf-8 -*-
"""
Created on Fri Jul 19 21:33:23 2024
...
for dataset in ["Renal PAS glomeruli/", "Renal PAS tubuli/"]:
    ...
    for subset in tqdm(["test", "val"]):
        ...
        PERT_std_C2_intradice_array = []
        PERT_std_C1_intradice_array = []
        
        MC_std_C2_intradice_array = []
        MC_std_C1_intradice_array = []
        ...
        # for image in os.listdir(general_dataset_path + "DATASET/" + subset + "/" + "manual/"):
            ...
            if C1_exist and C2_exist:
                ...
                PERT_std_C1_intradice = np.nanstd(PERT_dice_mat_C1)
                PERT_std_C1_intradice_array.append(PERT_std_C1_intradice)
                
                PERT_dice_mat_C2 = wjb_functions.intradice_mat(PERT_mask_matrix_C2)
                
                PERT_std_C2_intradice = np.nanstd(PERT_dice_mat_C2)
                PERT_std_C2_intradice_array.append(PERT_std_C2_intradice)
                ...
                MC_std_C1_intradice = np.nanstd(MC_dice_mat_C1)
                MC_std_C1_intradice_array.append(MC_std_C1_intradice)
                
                MC_dice_mat_C2 = wjb_functions.intradice_mat(MC_mask_matrix_C2)
                
                MC_std_C2_intradice = np.nanstd(MC_dice_mat_C2)
                MC_std_C2_intradice_array.append(MC_std_C2_intradice)
                ...
                plt.subplot(221)
                plt.title("PERT - C1, std dice = " + str(PERT_std_C1_intradice)[:5] + ", SI dice = " + str(SI_GT_C1_dice)[:5])
                plt.imshow(PERT_dice_mat_C1)
                plt.colorbar(fraction=0.046, pad=0.04)
                plt.subplot(222)
                plt.title("PERT - C2, std dice = " + str(PERT_std_C2_intradice)[:5] + ", SI dice = " + str(SI_GT_C2_dice)[:5])
                plt.imshow(PERT_dice_mat_C2)
                plt.colorbar(fraction=0.046, pad=0.04)
                plt.subplot(223)
                plt.title("MC - C1, std dice = " + str(MC_std_C1_intradice)[:5] + ", SI dice = " + str(SI_GT_C1_dice)[:5])
                plt.imshow(MC_dice_mat_C1)
                plt.colorbar(fraction=0.046, pad=0.04)
                plt.subplot(224)
                plt.title("MC - C2, std dice = " + str(MC_std_C2_intradice)[:5] + ", SI dice = " + str(SI_GT_C2_dice)[:5])
            ...
            if C2_exist and not C1_exist:
                ...
                plt.colorbar(fraction=0.046, pad=0.04)
                plt.subplot(212)
                plt.title("PERT - C2, std dice = " + str(MC_std_C2_intradice)[:5] + ", SI dice = " + str(SI_GT_C2_dice)[:5])
                plt.imshow(MC_dice_mat_C2)
                plt.colorbar(fraction=0.046, pad=0.04)
                plt.savefig(path_to_save_figure+"std_intradice_maps_"+image)
                plt.close()
            
        PERT_std_C1_intradice_array = np.array(PERT_std_C1_intradice_array)
        SI_GT_C1_dice_array = np.array(SI_GT_C1_dice_array)
        
        PERT_rho_C1 = wjb_functions.pearsons(PERT_std_C1_intradice_array,SI_GT_C1_dice_array)[0][1]
        
        PERT_std_C2_intradice_array = np.array(PERT_std_C2_intradice_array)
        SI_GT_C2_dice_array = np.array(SI_GT_C2_dice_array)
        
        PERT_rho_C2 = wjb_functions.pearsons(PERT_std_C2_intradice_array,SI_GT_C2_dice_array)[0][1]
        
        MC_std_C1_intradice_array = np.array(MC_std_C1_intradice_array)
        
        MC_rho_C1 = wjb_functions.pearsons(MC_std_C1_intradice_array,SI_GT_C1_dice_array)[0][1]
        
        MC_std_C2_intradice_array = np.array(MC_std_C2_intradice_array)
        
        MC_rho_C2 = wjb_functions.pearsons(MC_std_C2_intradice_array,SI_GT_C2_dice_array)[0][1]
        
        
        plt.figure(figsize=(16,15))
        plt.suptitle("Dataset: " + dataset[:-1] + ", subset: " + subset)
        plt.subplot(221)
        plt.title("PERT - C1: std intradice pert x SI-GT dice, rho = " + str(PERT_rho_C1)[:5])
        plt.scatter(PERT_std_C1_intradice_array,SI_GT_C1_dice_array)
        plt.xlabel("std perturbation intradice C1")
        plt.ylabel("single inference dice")
        plt.subplot(222)
        plt.title("PERT - C2: std intradice pert x SI-GT dice, rho = " + str(PERT_rho_C2)[:5])
        plt.scatter(PERT_std_C2_intradice_array,SI_GT_C2_dice_array)
        plt.xlabel("std perturbation intradice C2")
        plt.ylabel("single inference dice")
        plt.subplot(223)
        plt.title("MC - C1: std intradice MC x SI-GT dice, rho = " + str(MC_rho_C1)[:5])
        plt.scatter(MC_std_C1_intradice_array,SI_GT_C1_dice_array)
        plt.xlabel("std MCurbation intradice C1")
        plt.ylabel("single inference dice")
        plt.subplot(224)
        plt.title("MC - C2: std intradice MC x SI-GT dice, rho = " + str(MC_rho_C2)[:5])
        plt.scatter(MC_std_C2_intradice_array,SI_GT_C2_dice_array)
        plt.xlabel("std MCurbation intradice C2")
        plt.ylabel("single inference dice")
